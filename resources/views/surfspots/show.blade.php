...
@section('content')
    <div class="container mt-3 main">
        ...
        <div class="card spots-details">
            <div class="card-body">
                <h1 class="text-center mb-4">{{ $surfspot->name }}の詳細</h1>
                <dl>            
                    <dt>画像：</dt>
                    <dd class="show-dd"><img src="{{ asset('images/' . $surfspot->image) }}" alt="{{ $surfspot->name }}" class="img-fluid"></dd>

                    <dt>場所:</dt>
                    <dd>{{ $surfspot->address }}</dd>
                    
                    <dt>説明:</dt>
                    <dd>{{ $surfspot->description }}</dd>
                </dl>
            </div>
        </div>
        ...
        @auth
            <form action="{{ route('reviews.store') }}" method="POST" class="mt-3 mb-5">
                @csrf
                <input type="hidden" name="surf_spot_id" value="{{ $surfspot->id }}">
        ...
        <div class="d-flex justify-content-between spot-button back-button">
            <a href="{{ route('surfspots.index') }}" class="btn btn-secondary">← 戻る</a>
